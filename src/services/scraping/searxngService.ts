...
export async function searchNewsRealtime(
  ...
  try {
    ...
    const newsResults = searchResults.results
      .filter((result: any) => isNewsSource(result.url) || result.category === 'news')
      .map((result: any) => ({
        ...result,
...
export async function searchFactCheckSources(claim: string) {
  ...
  try {
    ...
    return {
      factChecks: results.results.map((result: any) => ({
        title: result.title,
        url: result.url,
...
export async function searchEventTimeline(topic: string) {
  ...
  for (const query of timelineQueries) {
    try {
      ...
      timelineData.push(...results.results.map((result: any) => ({
        title: result.title,
